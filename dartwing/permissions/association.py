...
"""
Permission utilities for Association DocType.

This module implements organization-scoped access control for Association records.
Users can only access Association documents belonging to Organizations they have
been granted access to via User Permissions.

Implements:
- FR-004: Filter list views to show only permitted associations
...
def get_permission_query_conditions(user: str = None) -> str:
    """
    Return SQL conditions to filter Association list based on user's org access.

    This is called by Frappe when listing Association documents to filter
    ...
    # Filter to associations in user's organizations
    # frappe.db.escape() already returns a quoted string
    org_list = ", ".join(frappe.db.escape(org) for org in organizations)
    return f"`tabAssociation`.`organization` IN ({org_list})"


def has_permission(doc, ptype: str = "read", user: str = None) -> bool:
    """
    Check if user has permission on a specific Association document.

    Args:
        doc: The Association document to check
    ...
    # Check if user has access to this Association's organization
    if not doc.organization:
        ...
        frappe.log_error(
            f"Association {doc.name} has no organization - allowing access",
            "Permission Warning"
        )
        return True
