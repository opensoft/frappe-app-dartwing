...
"""
Permission utilities for Company DocType.

This module implements organization-scoped access control for Company records.
Users can only access Company documents belonging to Organizations they have
been granted access to via User Permissions.

Implements:
- FR-004: Filter list views to show only permitted companies
...
def get_permission_query_conditions(user: str = None) -> str:
    """
    Return SQL conditions to filter Company list based on user's org access.

    This is called by Frappe when listing Company documents to filter
    ...
    # Filter to companies in user's organizations
...
def has_permission(doc, ptype: str = "read", user: str = None) -> bool:
    """
    Check if user has permission on a specific Company document.

    Args:
        doc: The Company document to check
    ...
    if not doc.organization:
        ...
        frappe.log_error(
            f"Company {doc.name} has no organization - allowing access",
            "Permission Warning"
        )
        return True
